...
cluster.authenticate('development1','dev@ilink08')
var q = require('q');
...
var _addDocument = function(key, data, bucketName) {
	...
	promise.then(function(cas) {
		...
		logger.error(util.format('could not add document with key:%s, error:%j', key, error));
...
var _addDocumentWithTtl = function(key, data, ttl, bucketName) {
	...
	promise.then(function(cas) {
		...
		logger.error(util.format('could not add document with ttl with key:%s, error:%j', key, error));
		deffered.reject(error);
	});
	return deffered.promise;
};
...
var _getDocument = function(key, withCas, bucketName) {
	...
	promise.then(function(doc) {
		...
		logger.error(util.format('could not find document with key:%s, error:%j', key, error));
		deffered.reject(error);
	});
	return deffered.promise;
};
...
var _getDocuments = function(keys, withCas, bucketName) {
	var deffered = q.defer();
	var getMulti = q.denodeify(db.getMulti);
	var promise = getMulti(keys, bucketName);
	promise.then(function(results) {
		...
		logger.error(util.format('could not find documents with keys:%j, error:%j', keys, error));
		deffered.reject(error);
	});
	return deffered.promise;
};
...
var _replaceDocument = function(key, doc, cas, bucketName) {
	...
	promise.then(function(result) {
		...
		logger.error(util.format('could not replace document with key:%s, error:%j', key, error));
		deffered.reject(error);
	});
	return deffered.promise;
};
...
var _removeDocument = function(key, bucketName) {
	...
	promise.then(function(result) {
		...
		logger.error(util.format('could not delete document with key:%s, error:%j', key, error));
		deffered.reject(error);
	});
	return deffered.promise;
};
...
var _getNextUserCounterKey = function(bucketName) {
	var deffered = q.defer();
	var counter = q.denodeify(db.counter);

	var promise = counter('count::user', 1, {
		initial : 1
	}, bucketName);
	promise.then(function(result) {
		deffered.resolve(result);
...
/**
 * To generate vendor control prefix
 * 
 * @param bucketName
 *            name of the bucket
 */
var _getNextVendorControlCounterKey = function(bucketName) {
	var deffered = q.defer();
	var counter = q.denodeify(db.counter);
...
module.exports = {
	...
	getNextUserCounterKey : _getNextUserCounterKey,
	getNextVendorControlCounterKey : _getNextVendorControlCounterKey
};